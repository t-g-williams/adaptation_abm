...
def main(mods, exp_name, save=True):
    if save == True:
    ...
    timeline(mods, exp_name, savedir)
...
def timeline(mods, exp_name, savedir):
    ...
    if len(mods) <= 5:
        ...
        lss = ['-']*(len(mods)-1)
    lw = 3
    titles = ['Change in wealth','Change in icome','Change in SOM','Change in rangeland biomass']
    ...
    fig.savefig(savedir+'combined_effect.png')
    plt.close('all')
